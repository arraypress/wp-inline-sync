...
namespace ArrayPress\InlineSync\Utils;
 ...
 * prefix ("MyPlugin\ArrayPress\InlineSync\Utils"), unique per plugin
...
final class Runtime {
	 ...
	 */
	private const LIBRARY = 'inline-sync';

	/**
	 * The per-build prefix.
	 *
	 * "inline-sync" for a plain Composer install — development, or a single
	 * consumer that does not use Strauss — and "{prefix}-inline-sync" for a
	 * prefixed build.
	 *
	 * @return string
	 */
	 ...
	 *
	 * Two prefixed copies registering `inline-sync/v1` do not collide loudly
	 * -- the second registration wins and the first plugin's routes quietly
	 * resolve to the second plugin's callbacks, against the second plugin's
	 * registry, which knows nothing about the first plugin's syncs.
	 *
	 * @return string
	 */
